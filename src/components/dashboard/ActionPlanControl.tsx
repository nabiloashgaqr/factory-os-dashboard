...
import ActionPlanExecutionFunnel from "@/components/charts/ActionPlanFunnel";

function isOpen(status: string) { return !/complete|closed|done/i.test(status); }
function isOverdue(endDate: string, status: string) { return endDate && new Date(endDate) < new Date() && isOpen(status); }
function priorityColor(p: string) { if (/crit/i.test(p)) return "var(--critical)"; if (/high/i.test(p)) return "var(--warning)"; return "var(--accent)"; }
...
export default function ActionPlanControl() {
  ...
  const stats = useMemo(() => {
    ...
    const avgExec = actions.length ? actions.reduce((s, a) => s + a.executionPct, 0) / actions.length : 0;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <SectionHeader
        title={t.actions}
        subtitle={language === "ar" ? "الربط مع Action Plans لمواجهة انحرافات المؤشرات" : "Live link to Action Plans DB"}
        icon={<CheckSquare className="text-[var(--success)]" />}
        right={<button onClick={() => downloadCsv("action_plans.csv", actions as unknown as Record<string, unknown>[])} className="flex items-center gap-2 text-xs bg-[var(--border)] hover:bg-[var(--accent)] hover:text-[var(--bg)] px-3 py-2 rounded-lg font-medium transition-colors"><Download size={14} /> CSV</button>}
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 fos-stagger">
        ...
        <StatCard label={language === "ar" ? "إجمالي العائد" : "Total Projected ROI"} value={formatCurrency(stats.totalRoi)} accent="var(--success)" />
      </div>

      {/* Action Plan Execution Funnel */}
      <ActionPlanExecutionFunnel />

      <Card className="p-5">
        <h3 className="text-sm font-semibold mb-3">{language === "ar" ? "الخطط حسب الأولوية" : "Actions by Priority"}</h3>
        <div className="h-64">
          {byPriority.length === 0 ? <EmptyState message={t.noData} /> : (
      ...
      <Card className="p-5 overflow-x-auto">
        <h3 className="text-sm font-semibold mb-3">{language === "ar" ? "جدول خطط العمل" : "Action Plans"}</h3>
        {actions.length === 0 ? <EmptyState message={t.noData} /> : (
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="text-xs font-bold opacity-60 border-b border-[var(--border)]">
                <th className="py-3 px-2 text-center">ID</th>
                <th className="py-3 px-2 text-start">{language === "ar" ? "المبادرة" : "Initiative"}</th>
                <th className="py-3 px-2 text-start">{language === "ar" ? "المؤشر" : "Target KPI"}</th>
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {actions.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-[var(--bg)] transition-colors" style={overdue ? { backgroundColor: "color-mix(in srgb, var(--critical) 6%, transparent)" } : undefined}>
                    <td className="py-4 px-2 text-center font-mono text-sm font-bold text-[var(--accent)]">{shortRef(a.id, "AP")}</td>
                    <td className="py-4 px-2 font-extrabold leading-relaxed max-w-sm">
                      <span className="flex items-center gap-1.5">
                        {(overdue || a.escalationRequired) && <AlertTriangle size={14} className="text-[var(--critical)] flex-shrink-0" />}
                        {a.initiative}
                      </span>
                    </td>
                    <td className="py-4 px-2 font-bold opacity-95">{getCleanKpiName(a.targetKpi, language)}</td>
                    <td className="py-4 px-2 opacity-80">{a.owner}</td>
                    <td className="py-4 px-2 text-center">{a.status}</td>
                    <td className="py-4 px-2 text-center"><Badge color={priorityColor(a.priority)}>{a.priority}</Badge></td>
                    <td className="py-4 px-2 text-end font-mono text-base font-black">{a.executionPct}%</td>
                    <td className="py-4 px-2 text-end font-mono text-base font-black text-[var(--success)]">{formatCurrency(a.projectedRoi)}</td>
                  </tr>
                );
              })}
            </tbody>
