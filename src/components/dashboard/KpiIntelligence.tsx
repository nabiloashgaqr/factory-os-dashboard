...
export default function KpiIntelligence() {
  ...
  // Trend: average actual per date (across all KPIs, normalised by attainment %)
  const trend = useMemo(() => {
    ...
    return Object.keys(byDate)
      ...
      .map((d) => ({
        date: d.slice(5),
        attainment: Number((byDate[d].sum / byDate[d].count).toFixed(1)),
      }));
  }, [rows]);
  ...
  const byLine = useMemo(() => {
    ...
    rows.forEach((r) => {
      map[r.line] = (map[r.line] || 0) + 1;
    });
    return Object.keys(map).map((line) => ({ line, readings: map[line] }));
  }, [rows]);

  const alertsTable = useMemo(
    () => rows.filter((r) => r.alertLevel === "Warning" || r.alertLevel === "Critical" || r.actionRequired),
    [rows]
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <SectionHeader
        ...
        subtitle={
          language === "ar"
            ? "التحليل الإحصائي وتوزيع التنبيهات ورصد القراءات الشاذة"
            : "Statistical analysis, alert distribution & anomaly detection"
        }
        icon={<BarChart2 className="text-[var(--accent)]" />}
        right={
          <button
            onClick={() => downloadCsv("kpi_measurements.csv", rows as unknown as Record<string, unknown>[])}
      ...
      <ContextualAI
        pageContext="kpi_analysis"
        currentData={{ alertCounts, totalReadings: rows.length, byLine }}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-5">
          <h3 className="text-sm font-semibold mb-3">
            {language === "ar" ? "توزيع حالة التنبيهات" : "Alert Distribution"}
          </h3>
          <div className="h-56">
            {pieData.length === 0 ? (
        ...
        <Card className="p-5 lg:col-span-2">
          <h3 className="text-sm font-semibold mb-3">
            {language === "ar" ? "اتجاه نسبة بلوغ الهدف عبر الزمن" : "Target Attainment Trend (%)"}
          </h3>
          <div className="h-56">
            {trend.length === 0 ? (
              <EmptyState message={t.noData} />
      ...
      <Card className="p-5">
        <h3 className="text-sm font-semibold mb-3">
          {language === "ar" ? "عدد القراءات حسب خط الإنتاج" : "Readings by Production Line"}
        </h3>
        <div className="h-56">
          {byLine.length === 0 ? (
            <EmptyState message={t.noData} />
      ...
      <Card className="p-5 overflow-x-auto">
        <h3 className="text-sm font-semibold mb-3">
          {language === "ar" ? "جدول القراءات الحرجة والتحذيرية" : "Warning / Critical Readings"}
        </h3>
                ...
                >
                  ...
                  <td className="py-2 text-center">
                    <span
                      ...
                      style={{
                        color: alertColorVar(r.alertLevel),
                        backgroundColor: `color-mix(in srgb, ${alertColorVar(r.alertLevel)} 14%, transparent)`,
                      }}
                    >
                      {r.alertLevel}
                    </span>
