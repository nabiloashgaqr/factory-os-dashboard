...
export default function ParetoAnalysis() {
  ...
  // Pareto from root-cause frequency of warning/critical readings.
  const pareto = useMemo(() => {
    const map: Record<string, number> = {};
    rows
      .filter((r) => r.alertLevel === "Warning" || r.alertLevel === "Critical")
      .forEach((r) => {
        const cause = r.rootCause?.trim() || (language === "ar" ? "غير مصنّف" : "Uncategorized");
        map[cause] = (map[cause] || 0) + 1;
      });
    const sorted = Object.keys(map)
      .map((cause) => ({ cause, count: map[cause] }))
      .sort((a, b) => b.count - a.count);
    const total = sorted.reduce((s, x) => s + x.count, 0) || 1;
    let cum = 0;
    return sorted.map((x) => {
      cum += x.count;
      return { ...x, cumulative: Number(((cum / total) * 100).toFixed(1)) };
    });
  }, [rows, language]);

  // OEE loss decomposition (illustrative split using avg OEE).
  const oeeLoss = useMemo(() => {
    const oeeRows = rows.filter((r) => /oee/i.test(r.kpiName));
    const avg = oeeRows.length
    ...
    return [
      ...
      { name: language === "ar" ? "خسائر الجودة" : "Quality Scrap", value: Number((loss * 0.18).toFixed(1)), color: "var(--accent)" },
      { name: language === "ar" ? "الكفاءة الصافية" : "Net OEE", value: Number(avg.toFixed(1)), color: "var(--success)" },
    ];
  }, [rows, language]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <SectionHeader
        ...
        subtitle={
          language === "ar"
            ? "تصنيف 80/20 للجذور المسببة ومخطط تفكيك خسائر الكفاءة الإجمالية"
            : "80/20 root-cause ranking & OEE loss decomposition"
        }
        icon={<TrendingDown className="text-[var(--accent)]" />}
      />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-5">
          <h3 className="text-sm font-semibold mb-3">
            {language === "ar" ? "تحليل باريتو للتوقفات الحرجة" : "Downtime Pareto (80/20)"}
          </h3>
          <div className="h-72">
            {pareto.length === 0 ? (
        ...
        <Card className="p-5">
          <h3 className="text-sm font-semibold mb-3">
            {language === "ar" ? "تفكيك خسائر الكفاءة OEE" : "OEE Loss Decomposition"}
          </h3>
