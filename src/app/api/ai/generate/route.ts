...
// Identical (provider + model + prompt + context) requests within the TTL
// reuse the previous answer instead of spending another API call.
...
interface GenResult {
  text: string;
  status: number; // upstream status
  error?: string;
}

// ── Provider drivers (all via REST — no heavy SDKs) ─────────────────────

async function runGemini(
  apiKey: string,
  model: string,
  fullPrompt: string,
  cfg: { temperature: number; maxOutputTokens: number; json?: boolean }
): Promise<GenResult> {
  ...
  if (!r.ok) {
    return { text: "", status: r.status, error: data?.error?.message };
  }
  const text =
  ...
  try {
    ...
    if (provider === "disabled") {
      return NextResponse.json(
        { success: false, error: "AI is disabled in Settings." },
        { status: 400 }
      );
    }
    if (!["gemini", "openai", "claude"].includes(provider)) {
      return NextResponse.json(
        { success: false, error: `Unknown AI provider: ${provider}` },
        { status: 400 }
      );
    }
    ...
    if (!apiKey) {
      return NextResponse.json(
        { success: false, error: `${provider} API key is missing.` },
        { status: 401 }
      );
    }
    if (!prompt) {
    ...
    const summary =
      contextData !== undefined ? JSON.stringify(contextData).slice(0, 12000) : "";
    ...
    if (result.status !== 200) {
      ...
      if (quota) {
        msg = `${provider} quota/rate limit exceeded. Wait a minute, lower the auto-sync frequency, or enable billing for your account.`;
      }
      return NextResponse.json(
        { success: false, error: msg, quota },
