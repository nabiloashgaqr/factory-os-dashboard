...
/** Returns the first related page id from a Notion relation property, if any. */
export function getRelationId(prop: any): string {
  if (prop && Array.isArray(prop.relation) && prop.relation.length) {
    return prop.relation[0].id as string;
  }
  return "";
}
